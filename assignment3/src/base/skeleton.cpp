...
void Skeleton::setJointRotation(unsigned index, Vec3f euler_angles) {
	...
	Mat3f rotation1, rotation2, rotation3;
	rotation1 = Mat3f::rotation(Vec3f(1, 0, 0), euler_angles[0]);
	rotation2 = Mat3f::rotation(Vec3f(0, 1, 0), euler_angles[1]);
	rotation3 = Mat3f::rotation(Vec3f(0, 0, 1), euler_angles[2]);

	Mat3f rotation = rotation1 * rotation2 * rotation3;
...
void Skeleton::updateToWorldTransforms(unsigned joint_index, const Mat4f& parent_to_world) {
	...
	int i;
	for (i = 0; i < joints_[joint_index].children.size(); i++) {
