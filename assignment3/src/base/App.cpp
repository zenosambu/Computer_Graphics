...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		int papa = skel_.getJointParent(i);
		if (papa != -1) {
			glColor3f(252, 252, 252);
			Vec3f eske = (transforms[papa] * Vec4f(0, 0, 0, 1)).getXYZ();
			glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
			glVertex3f(eske[0],eske[1],eske[2]);
		}
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for(const auto& sv : source_vertices){
		Vec3f parziale_pos = Vec3f();
		Vec3f parziale_norma = Vec3f();
		...
		// For R5, transform the normals as well.
			int joint_1 = sv.joints[i];
			Vec3f medin = ssd_transforms[joint_1] * sv.position;
			parziale_pos += sv.weights[i] * medin;
			Vec3f normin = ssd_transforms[joint_1].getXYZ() * sv.normal;
			parziale_norma += sv.weights[i] * normin;
		}
		v.position = parziale_pos;
		v.normal = parziale_norma;
		v.color = sv.color;
