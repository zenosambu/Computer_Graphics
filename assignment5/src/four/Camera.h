...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		float coord_x = (pixel[0] / ((float)imageSize[0] / 2)) - 1;
		float coord_y = (-pixel[1] / ((float)imageSize[1] / 2)) + 1;
		return FW::Vec2f(coord_x, coord_y);
	}
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		FW::Vec3f new_origine = this->center + point[0] * (this->size * this->horizontal) / 2.0 + point[1] * (this->size * this->up) / 2.0;

		return Ray(new_origine, this->direction);
	}
...
public:
	...
	FW::Vec3f genitore1 = this->center;
	float d = (1.0f / FW::tan(fov_angle / 2.0f));
	FW::Vec3f genitore2 = (point.x * this->horizontal + point.y * this->up + d*this->direction);
	genitore2.normalize();
	
	

		return Ray( genitore1, genitore2 );
	}
