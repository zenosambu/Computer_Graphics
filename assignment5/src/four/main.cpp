	...
	for (int j = 0; j < args.height; ++j) {
		...
		for (int i = 0; i < args.width; ++i) {
			...
			for (int n = 0; n < args.num_samples; ++n) {
				...
				image->setVec4f(Vec2i(i,j), Vec4f(sample_color_sum/(float)args.num_samples, 1));
				if (depth_image) {
					// YOUR CODE HERE (R2)
					...
					if ((hit.t >= args.depth_min) && (hit.t <= args.depth_max)) {
						f = 1.0 - ((float)hit.t - args.depth_min)/(args.depth_max - args.depth_min);
					}
